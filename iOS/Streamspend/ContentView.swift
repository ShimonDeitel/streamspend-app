...
struct ContentView: View {
    ...
    @State private var editingEntry: StreamspendEntry?
    ...
    private var emptyState: some View {
        VStack(spacing: 12) {
            ...
            Text("No entries yet")
                ...
                .foregroundStyle(Theme.textPrimary)
            Text("Log every streaming and app subscription to see the real monthly total.")
                .font(Theme.captionFont)
                .foregroundStyle(Theme.textSecondary)
    ...
    private func row(for entry: StreamspendEntry) -> some View {
        HStack {
...
struct EntryEditView: View {
    ...
    var onSave: (StreamspendEntry) -> Void

    private let existingID: UUID?

    init(store: Store, existing: StreamspendEntry? = nil, onSave: @escaping (StreamspendEntry) -> Void) {
        self.store = store
        self.onSave = onSave
            ...
            .toolbar {
                ...
                ToolbarItem(placement: .confirmationAction) {
                    Button("Save") {
                        ...
                        let entry = StreamspendEntry(id: existingID ?? UUID(), title: title.isEmpty ? "Untitled" : title, amount: amount, date: date, note: note)
                        onSave(entry)
                        dismiss()
                    }
